...
const Template_Bill = ({visible, setVisible, Bill_Id}) => {
  const dataBills = useSelector(state => state.data_infor).data.bills;
  ...
  useEffect(() => {
    let tmp = [];
    bill[0].List_Room_Id.map(item => {
      dataRooms.map(item_room => {
        if (item == item_room.id) {
          tmp.push(item_room);
        }
      });
    });
